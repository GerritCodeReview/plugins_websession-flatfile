...
public class FlatFileWebSessionCacheCleaner implements Runnable {
  ...
  static class CleanerLifecycle implements LifecycleListener {
    ...
    @Inject
    CleanerLifecycle(final WorkQueue queue,
        final FlatFileWebSessionCacheCleaner fileCacheCleaner,
        @WebSessionMaxAge long maxAge) {
      this.queue = queue;
      this.cleaner = fileCacheCleaner;
    ...
    public void start() {
      queue.getDefaultQueue().scheduleAtFixedRate(cleaner, INITIAL_DELAY_MS,
          sessionMaxAgeMs * 2, MILLISECONDS);
    }
